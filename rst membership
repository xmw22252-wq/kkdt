// ==UserScript==
// @name         Rolster membership autofill (clean final)
// @namespace    local.rolster.bstage
// @version      4.2.0
// @description  Autofill Membership No/Name/DOB/Phone on Rolster bstage membership (clean, no debug UI)
// @match        https://rolster.bstage.in/shop/my/membership*
...
(() => {
  ...
  // ===== 你要替换成的值 =====
  const TARGET = {
    'Membership No': 'Y5EAMR',
    'Name': 'WEN WEN',
    'Date of Birth': '1996.07.25',
    'Phone Number': '+86-13228189464',
  };

  // ===== 当前页面上原本显示的会员号（你截图里的是这个）=====
  const ORIGINAL_MEMBER_NO = 'A3CGC6';
  ...
  function apply(ROOT) {
    ...
    // 先尝试按标签改会员号
    if (setValueByLabel(ROOT, 'Membership No', TARGET['Membership No'])) okCount++;
    else if (setValueByLabel(ROOT, 'Membership Number', TARGET['Membership No'])) okCount++;
    else if (setValueByLabel(ROOT, 'Member No', TARGET['Membership No'])) okCount++;
    else if (setValueByLabel(ROOT, 'Member ID', TARGET['Membership No'])) okCount++;
    // 如果没有标签，就直接把旧会员号替换掉
    else if (replaceExactText(ROOT, ORIGINAL_MEMBER_NO, TARGET['Membership No'])) okCount++;

    if (setValueByLabel(ROOT, 'Name', TARGET['Name'])) okCount++;
    if (setValueByLabel(ROOT, 'Date of Birth', TARGET['Date of Birth'])) okCount++;
    if (setValueByLabel(ROOT, 'Phone Number', TARGET['Phone Number'])) okCount++;
  ...
  (function mobileFlipFix() {
    ...
    function apply3DFix() {
      ...
      const candidates = Array.from(
        ...
        if (
          !(
            (t.includes('Membership No') ||
             t.includes('Membership Number') ||
             t.includes('Member No') ||
             t.includes('Member ID') ||
             t.includes('Name') ||
             t.includes(ORIGINAL_MEMBER_NO)) &&
